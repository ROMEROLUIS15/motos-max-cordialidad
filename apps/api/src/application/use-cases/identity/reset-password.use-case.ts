 ...
 * `$transaction` (inside the repository) to guarantee atomicity — a crash between
 * the two writes cannot leave the system in an inconsistent state.
 *
...
export class ResetPasswordUseCase {
  ...
  async execute(token: string, newPassword: string): Promise<{ message: string }> {
    ...
    const record = await this.tokens.findByTokenHash(hash);
    ...
    const user = await this.tokens.consumeAndUpdatePassword(record.id, record.userId, passwordHash);
